import { getSession, type Session } from '@server/lib/session.js'
...
export const withTextLogin = createMiddleware<{ Variables: AuthVariables }>(async (c, next) => {
  const sessionId = getCookie(c, 'session')
  if (!sessionId) return c.text('Unauthorized', 401)

  const session = await getSession(c, false)
  if (!session?.login) return c.text('Unauthorized', 401)
